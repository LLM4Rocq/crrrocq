...
- the command `get_state_at_pos(doc, line, char)` returns the state at a given position
- the command `ast(state, command)` returns the AST of `command` parsed at `state`
- the command `ast_at_pos(doc, line, char)` returns the AST at a given position
